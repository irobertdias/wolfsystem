...
export default function Home() {
  ...
  const handleLogin = async () => {
    ...
    if (data.user) {
      // Verifica se tem workspace criado
      ...
      // Se não tem workspace, verifica se está autorizado na tabela cadastros
      ...
      // Se não tem cadastro nem workspace, é admin — deixa entrar
      router.push("/crm");
    }
  };
...
>
  Esqueceu a senha?
          ...
          <button
            onClick={handleLogin}
            disabled={loading}
            style={{ background: loading ? "#86efac" : "black" }}
            className="w-full text-white rounded-lg py-3 text-sm font-semibold transition-colors"
          >
        ...
        <button
          ...
          className="w-full border-2 border-green-500 text-green-600 rounded-lg py-3 text-sm font-semibold hover:bg-green-50 transition-colors uppercase tracking-wide"
        >
