...
export default function CRM() {
  ...
  const menuItems = [
    ...
    ...(isAdmin
      ...
      : [{ path: "/crm/contatos", icon: "👥", label: "Contatos", badge: 0 }]
    ),
    { path: "/crm/configuracoes", icon: "⚙️", label: "Configurações", badge: 0 },
  ];
    ...
    <div style={{ display: "flex", height: "100vh", fontFamily: "Arial, sans-serif", background: "#0a0a0a" }}>
      <div style={{ width: 220, background: "#111", borderRight: "1px solid #1f2937", display: "flex", flexDirection: "column", padding: 16, gap: 8, flexShrink: 0 }}>
        ...
        {menuItems.map(item => (
          <button key={item.path} onClick={() => router.push(item.path)}
            style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: pathname?.startsWith(item.path) ? "#16a34a22" : "none", border: "none", borderRadius: 8, cursor: "pointer", color: pathname?.startsWith(item.path) ? "#16a34a" : "#9ca3af", fontSize: 13, fontWeight: pathname?.startsWith(item.path) ? "bold" : "normal", textAlign: "left" }}>
            <span>{item.icon}</span>
            {item.label}
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 32 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", flexDirection: "column", gap: 16 }}>
          ...
          <h2 style={{ color: "white", fontSize: 18, fontWeight: "bold", margin: 0 }}>Selecione uma opção no menu</h2>
          <p style={{ color: "#6b7280", fontSize: 14, margin: 0 }}>Escolha uma das abas ao lado para começar</p>
          <button onClick={() => router.push("/crm/dashboard")} style={{ background: "#16a34a", color: "white", border: "none", borderRadius: 8, padding: "12px 24px", fontSize: 14, cursor: "pointer", fontWeight: "bold", marginTop: 8 }}>📊 Ir para Dashboard</button>
        </div>
      </div>
    </div>
  );
