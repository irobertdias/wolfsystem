      ...
      {!status.contaSelecionada ? <div className={styles.empty}><strong>Escolha uma conta de anúncios</strong>Selecione acima a conta que este workspace acompanhará.</div> : dashboard ? <>
        ...
        <div className={styles.split}>
          <div className={styles.section}>
            ...
            <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Campanha</th><th>Status</th><th>Objetivo</th><th>Investimento</th><th>Leads</th><th>CPL</th><th>Conversas</th><th>Cliques</th><th>CTR</th><th>ROAS</th></tr></thead><tbody>
              {dashboard.campanhas.length === 0 ? <tr><td colSpan={10}><div className={styles.empty}>Nenhuma campanha encontrada neste período.</div></td></tr> : dashboard.campanhas.map(c => { const [label,statusClass]=traduzStatus(c.status); return <tr key={c.id}><td><div className={styles.campaignName} title={c.nome}>{c.nome}</div></td><td><span className={`${styles.status} ${statusClass}`}>{label}</span></td><td>{objetivo(c.objetivo)}</td><td>{money.format(c.gasto)}</td><td>{integer.format(c.leads)}</td><td>{money.format(c.cpl)}</td><td>{integer.format(c.conversas)}</td><td>{integer.format(c.cliques)}</td><td>{decimal(c.ctr)}%</td><td>{decimal(c.roas)}x</td></tr>; })}
            </tbody></table></div>
          </div>
          <div className={styles.section}>
