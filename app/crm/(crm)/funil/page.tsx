...
// ───────────────────────────────────────────────────────────────────────────
// Em vez de hard-codar "operadora / plano / valor_plano / status_venda"
// (que é telecom), o funil LÊ a mesma configuração de campos do Editor de
// Proposta. Assim:
//   • Telecom      → status=status_venda, valor=valor_plano, dims=operadora/plano
//   • Cobrança     → status=situacao,     valor=valor_divida, dims=carteira/faixa
//   • Advocacia    → status=fase,         valor=valor_causa,  dims=area/comarca
//   • Contabilidade→ status=etapa,        valor=honorario,    dims=regime/serviço
//
// O workspace só configura os campos no Editor; o funil se adapta sozinho,
// com auto-detecção + override manual persistido por workspace.
// ═══════════════════════════════════════════════════════════════════════════
...
type FunilConfig = {
  campoStatus: string;          // slug do campo que representa a etapa/status
  campoValor: string;           // slug do campo monetário principal
...
const PERIODOS: { key: PeriodoKey; label: string; curto: string; dias: number; icone: string; cor: string }[] = [
  { key: "semanal",    label: "Últimos 7 dias",  curto: "7d",   dias: 7,    icone: "📅", cor: "#16a34a" },
...
// Paleta pra séries/dimensões
const PALETA = ["#3b82f6", "#16a34a", "#f59e0b", "#8b5cf6", "#ec4899", "#06b6d4", "#dc2626", "#14b8a6", "#a855f7", "#0ea5e9", "#f97316", "#84cc16"];
...
const formatDataHora = (d: string | Date) => {
...
// Lê o valor de um campo de uma proposta, seja ele coluna fixa ou custom (jsonb)
...
// ─── localStorage da config do funil (por workspace) ────────────────────────
const FUNIL_CFG_KEY = (wsId: string) => `funil_config_v1__${wsId}`;

const carregarConfigSalva = (wsId: string): Partial<FunilConfig> | null => {
  if (!wsId || typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(FUNIL_CFG_KEY(wsId));
    return raw ? JSON.parse(raw) : null;
  } catch { return null; }
};

const salvarConfig = (wsId: string, cfg: FunilConfig) => {
  if (!wsId || typeof window === "undefined") return;
  try { window.localStorage.setItem(FUNIL_CFG_KEY(wsId), JSON.stringify(cfg)); } catch { /* noop */ }
};
...
const autoDetectarConfig = (campos: CampoUni[]): FunilConfig => {
  ...
  // Status: prioriza slug conhecido, senão primeiro dropdown com opções "de etapa"
  ...
  // Valor: prioriza moeda; entre as moedas, a de slug "valor"/"honorario"/"divida"/"causa"
  ...
  // Data: prioriza slug "data_proposta"/"data"/"abertura"; senão primeira data
  const dataPreferida =
...
export default function Funil() {
  ...
  const [etapaAberta, setEtapaAberta] = useState<string | null>(null); // opção do status em drilldown
  const [dimensaoSel, setDimensaoSel] = useState<string>("");          // slug da dimensão na aba "dimensoes"
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (wsDono) {
        ...
        if (uw?.workspace_id) {
          ...
          if (wsSub) {
            ...
            wsNome = wsSub.nome || "";
            wsUsername = wsSub.username || uw.workspace_id;
      ...
      setWorkspaceId(wsUsername || wsIds[0]);

      // 1) Carrega config de campos (igual ao Editor)
      let camposUnificados: CampoUni[] = [];
      try {
        const [respConfig, respCustom] = await Promise.all([
          supabase.from("proposta_campos_padrao_config").select("*").eq("workspace_id", wsUsername || wsIds[0]),
          supabase.from("proposta_campos_customizados").select("*").eq("workspace_id", wsUsername || wsIds[0]).eq("ativo", true).order("ordem", { ascending: true }),
        ]);
        ...
        console.warn("[Funil] não consegui carregar config de campos, usando fallback telecom.", e);
        // Fallback mínimo telecom pra não quebrar
        camposUnificados = [
          { slug: "status_venda", label: "Status da Venda", tipo: "dropdown", origem: "fixo", visivel: true, ordem: 1, opcoes: ["GERADA", "AGUARDANDO AUDITORIA", "PENDENTE", "INSTALADA", "CANCELADA"] } as any,
          { slug: "valor_plano", label: "Valor", tipo: "moeda", origem: "fixo", visivel: true, ordem: 2, opcoes: null } as any,
          { slug: "plano", label: "Plano", tipo: "dropdown", origem: "fixo", visivel: true, ordem: 3, opcoes: [] } as any,
          { slug: "operadora", label: "Operadora", tipo: "dropdown", origem: "fixo", visivel: true, ordem: 4, opcoes: [] } as any,
          { slug: "data_proposta", label: "Data da Proposta", tipo: "data", origem: "fixo", visivel: true, ordem: 5, opcoes: null } as any,
        ];
      }
      setCampos(camposUnificados);

      // 2) Define config semântica (salva ou auto-detectada)
      const salva = carregarConfigSalva(wsUsername || wsIds[0]);
      ...
      // 3) Dimensão default pra aba de breakdown (primeiro dropdown que não é o status)
  ...
  // Opções do status (etapas do funil)
  const opcoesStatus = useMemo<string[]>(() => {
    ...
    // Se não tem opções definidas, deriva dos dados
  ...
  // Dimensões disponíveis (campos dropdown/checkbox visíveis, exceto o status)
  ...
  // Campos de valor (moeda/numero) disponíveis
  ...
  // ─── HELPERS SEMÂNTICOS (dependem da config) ────────────────────────────────
  ...
  const metricas = useMemo(() => {
    ...
    // Ciclo médio (criação → última atualização) dos ganhos
    let ciclo = 0;
    if (ganhos.length > 0) {
  ...
  const etapas = useMemo(() => {
    const arr = opcoesStatus.map((opt, i) => {
  ...
  // Etapas "de pipeline" ordenadas pra exibir o funil (exclui perdidos do corpo)
  ...
  // ─── BREAKDOWN POR DIMENSÃO SELECIONADA ─────────────────────────────────────
  ...
  // Breakdown da etapa por cada dimensão (pra mostrar no drilldown)
  ...
  // Histograma de valores da etapa
  const histogramaEtapa = useMemo(() => {
    if (propsEtapa.length === 0) return [];
    const valores = propsEtapa.map(valorDe);
    const max = Math.max(...valores, 1);
    // 5 faixas dinâmicas baseadas no max
    const step = max / 5;
  ...
  // Colunas pra mostrar na lista = campos marcados "mostrar_na_lista", senão os visíveis principais
  const colunasLista = useMemo<CampoUni[]>(() => {
    ...
    // fallback: nome, status, valor, vendedor (se existirem) + 2 dropdowns
  ...
  const salvarConfigFunil = (novo: FunilConfig) => {
    setConfig(novo);
    if (workspaceId) salvarConfig(workspaceId, novo);
  };
    ...
    return (
      <div id="drill-etapa" style={{ ...cardStyle, borderTop: `4px solid ${cor}`, overflow: "hidden", boxShadow: `0 4px 20px ${cor}15` }}>
        ...
        <div style={{ padding: isMobile ? 16 : 24, display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {/* Quebras por dimensão + por vendedor */}
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12 }}>
            ...
            {/* Por primeira dimensão disponível */}
            <div style={{ background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 12, padding: 14 }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 16 : 22 }}>
      ...
      <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", justifyContent: "space-between", alignItems: isMobile ? "stretch" : "center", gap: 14 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button onClick={() => setShowConfig(s => !s)}
            style={{ ...inputStyle, display: "flex", alignItems: "center", gap: 6, background: showConfig ? "#eff6ff" : "#ffffff", color: showConfig ? "#3b82f6" : "#6b7280", borderColor: showConfig ? "#bfdbfe" : "#e5e7eb" }}>
            ⚙️ Configurar funil
          </button>
        </div>
      </div>

      {/* ═══ PAINEL DE CONFIG (semântica do funil) ═══ */}
      {showConfig && config && (
        <div style={{ ...cardStyle, padding: isMobile ? 16 : 20, borderTop: "3px solid #3b82f6" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
            <h3 style={{ ...sectionTitleStyle, margin: 0, fontSize: 14 }}>
              ...
              Como ler este funil neste workspace
            </h3>
          ...
          <p style={{ color: "#6b7280", fontSize: 12, margin: "0 0 16px", lineHeight: 1.5 }}>
            O funil lê os campos que você criou no <b>Editor de Proposta</b>. Aqui você só diz o que cada coisa significa — qual campo é a <b>etapa</b>, qual é o <b>valor (R$)</b>, e quais situações contam como <b style={{ color: "#16a34a" }}>ganho</b> ou <b style={{ color: "#dc2626" }}>perdido</b>. Funciona pra qualquer ramo (telecom, cobrança, advocacia, contabilidade).
          </p>
      ...
      <div style={{ ...cardStyle, padding: isMobile ? 14 : 16, display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : `repeat(${Math.min(4, 2 + dimensoesDisponiveis.slice(0,2).length)}, 1fr)`, gap: 10, alignItems: "end" }}>
          ...
          {/* Filtros dinâmicos pelas 2 primeiras dimensões */}
      ...
      {loading ? (
        ...
        <>
          ...
          {aba === "visao" && (
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, 1fr)", gap: isMobile ? 10 : 14 }}>
                ...
                <KPI cor="#3b82f6" bg="#eff6ff" icone="🎯" label="Pipeline" valor={formatBRLCompacto(metricas.valorPipeline)} titulo={formatBRL(metricas.valorPipeline)} sub={`${metricas.pipelineCount} em aberto`} isMobile={isMobile} />
          ...
          {/* ════════════ ABA: ETAPAS (drilldown protagonista) ════════════ */}
          {aba === "etapas" && (
            <>
              <div style={{ ...cardStyle, padding: isMobile ? 14 : 18 }}>
                <p style={{ color: "#6b7280", fontSize: 12, margin: "0 0 12px", fontWeight: 600 }}>
                  Selecione uma etapa pra abrir o dashboard completo dela (valores, faixas, quebras, lista):
                </p>
          ...
          {/* ════════════ ABA: QUEBRAS (breakdown dinâmico) ════════════ */}
          {aba === "dimensoes" && (
            <>
              <div style={{ ...cardStyle, padding: isMobile ? 14 : 18 }}>
                <p style={{ color: "#6b7280", fontSize: 12, margin: "0 0 12px", fontWeight: 600 }}>
                  Quebra os números por qualquer campo de seleção que você configurou. Escolha a dimensão:
                </p>
                {dimensoesDisponiveis.length === 0 ? (
                  <p style={{ color: "#9ca3af", fontSize: 13, fontStyle: "italic" }}>
                    Nenhum campo do tipo "Seleção" configurado. Crie campos dropdown no Editor de Proposta (ex: Operadora, Plano, Área, Carteira) que eles aparecem aqui.
                  </p>
                ) : (
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              {dimensaoSel && breakdownDim.length > 0 && (
                <>
                  {/* Gráfico de barras: receita + pipeline por valor da dimensão */}
                  <div style={{ ...cardStyle, padding: isMobile ? 16 : 24 }}>
                    <h3 style={sectionTitleStyle}>
                          ...
                          {breakdownDim.map((d, i) => {
                            ...
                            return (
                              <tr key={d.valor} style={{ borderTop: "1px solid #f3f4f6", background: i % 2 === 0 ? "#ffffff" : "#fafbfc" }}>
                                ...
                                <td style={{ padding: "12px 16px", color: "#3b82f6", fontSize: 13, fontWeight: 700 }}>{d.qtd}</td>
                                ...
                                <td style={{ padding: "12px 16px", color: "#3b82f6", fontSize: 13, fontWeight: 600 }}>{formatBRL(d.pipeline)}</td>
                              </tr>
                            );
                          })}
                        </tbody>
                      ...
                      {vendedoresStats.map((v, i) => {
                        ...
                        return (
                          <tr key={v.email} style={{ borderTop: "1px solid #f3f4f6", background: i % 2 === 0 ? "#ffffff" : "#fafbfc" }}>
                            ...
                            <td style={{ padding: "14px", color: "#3b82f6", fontSize: 12, fontWeight: 600 }}>{formatBRLCompacto(v.pipeline)}</td>
                  ...
                  <div style={{ display: "flex", gap: 4 }}>
                    {(["dia", "semana", "mes"] as const).map(m => (
                      <button key={m} onClick={() => setAgrupTempo(m)}
                        style={{ ...chipStyle, background: agrupTempo === m ? "#eff6ff" : "#ffffff", color: agrupTempo === m ? "#3b82f6" : "#6b7280", borderColor: agrupTempo === m ? "#3b82f6" : "#e5e7eb", fontWeight: agrupTempo === m ? 700 : 600 }}>
              ...
              <div style={{ ...cardStyle, padding: isMobile ? 16 : 24 }}>
                ...
                <ResponsiveContainer width="100%" height={isMobile ? 220 : 280}>
                  <LineChart data={serieTemporal} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                    ...
                    <Line type="monotone" dataKey="geradas" stroke="#3b82f6" strokeWidth={2} dot={false} name="Gerados" />
