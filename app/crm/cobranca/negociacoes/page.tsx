...
// 🆕 multi-tenant: hooks do Wolf
import { useWorkspace } from "../../../hooks/useWorkspace";
import { useModulos, ModuloBloqueado } from "../../../hooks/useModulos";
// 🆕 ChatSection embedded foi removido nesta adaptação (ver comentário no bloco da aba "Atendimentos")
// import { ChatSection } from "../../../chatbot/_sections/ChatSection";

// ═══════════════════════════════════════════════════════════════════════════
// 💰 COBRANÇA — Wolf System (multi-tenant)
// ───────────────────────────────────────────────────────────────────────────
// Mantém 12 status de fatura e disparos. Filtra por workspace_id em TODAS
// as queries (vendido por workspace). Disparos passam pelo wolf-whatsapp
// existente (POST /api/whatsapp?rota=disparos/criar com origem="cobranca").
// Duas fontes:
...
export default function CobrancaPage() {
  ...
  // 🆕 multi-tenant: Wolf usa useWorkspace + useModulos (não tem o sistema antigo de escopos)
  const { wsId, user, workspace, wsPronto } = useWorkspace();
  const { modulos, carregado: modulosCarregados } = useModulos();

  const isSuperAdmin = user?.email === ADMIN_EMAIL;
  const isDono = !!workspace && workspace.owner_id === user?.id;
  // No Wolf, qualquer um do workspace com o módulo liberado tem acesso.
  // Super admin tem acesso sempre. Permissões granulares (mudar_status, juridico, etc)
  // ainda não existem no Wolf — dono e admin podem tudo.
  const podeMudarStatus = isSuperAdmin || isDono;
  const podeDisparar     = isSuperAdmin || isDono;
  const podeCancelar     = isSuperAdmin || isDono;
  const podeJuridico     = isSuperAdmin || isDono;
  const podeProtestada   = isSuperAdmin || isDono;
  const temAcesso = isSuperAdmin || !!modulos.cobranca;
  const permitido: boolean | null = (!wsPronto || !modulosCarregados) ? null : temAcesso;
  ...
  const fetchRespostasCobranca = async () => {
    ...
    try {
      // 1. desde quando vale: o PRIMEIRO disparo de cobrança DESSE workspace
  ...
  const salvarEdicaoCliente = async () => {
    ...
    const { error } = await supabase.from("proposta").update(payload)
  ...
  useEffect(() => {
    if (permitido !== true || !wsId) return;
    fetchTudo();

    // 🆕 multi-tenant: filtra realtime pelo workspace_id pra não receber mudanças de outros tenants
    ...
    try {
      ...
      if (errCount) {
        ...
        for (let i = 0; i < nPaginas; i++) {
          reqs.push(
            supabase.from("faturas_status").select("*")
              .eq("workspace_id", wsId)   // 🆕 multi-tenant
  ...
  const salvarStatusFatura = async (payload: any): Promise<{ message: string } | null> => {
    if (!wsId) return { message: "Workspace não identificado" };
    // 🆕 multi-tenant: sempre carrega workspace_id no payload (pro INSERT) e filtra no UPDATE
    const payloadComWs = { ...payload, workspace_id: wsId };
    try {
      // tenta atualizar as linhas existentes dessa fatura
      const { data: upd, error: e1 } = await supabase
        .from("faturas_status")
        .update(payloadComWs)
        .eq("workspace_id", wsId)   // 🆕 multi-tenant: só atualiza no mesmo workspace
      ...
      if (!upd || upd.length === 0) {
        const { error: e2 } = await supabase.from("faturas_status").insert(payloadComWs);
  ...
  const dispararCobranca = async () => {
    ...
    try {
      ...
      const resp = await fetch(`/api/whatsapp?rota=${rota}`, {
        ...
        body: JSON.stringify({
          workspaceId: wsId,   // 🆕 multi-tenant: backend valida que o canal pertence a esse ws
  ...
  // Loading permissão / workspace
  if (permitido === null) {
    return (
      <div style={{ minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: "#6b7280", fontSize: 13 }}>⏳ Verificando acesso...</div>
      </div>
    );
  }

  // 🆕 Sem acesso → tela de upsell padronizada do Wolf (com link pra falar com comercial)
  if (!permitido) {
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ color: "#6b7280", fontSize: 12, margin: "2px 0 0" }}>
              {/* 🆕 multi-tenant: mostra o workspace logado em vez do "Grupo Unita" fixo */}
              <b style={{ color: "#dc2626" }}>{workspace?.nome || "Wolf System"}</b> · Cobre direto do CRM ou suba uma planilha
            </p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <>
          ...
          {aba === "atendimentos" && (
            ...
            {/* ░░ ChatSection embutido foi REMOVIDO na adaptação pro Wolf.
                Motivo: o ChatSection do Wolf (chatbot/_sections/) não aceita
                prop "moduloFiltro" ainda. A lista de leads que responderam
                continua acima — e o botão "↩️ Responder no Atendimento" em
                cada conversa leva direto pro /chatbot pra atender. ░░ */}
            </>
          )}
