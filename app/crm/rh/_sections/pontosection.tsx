...
export function PontoSection() {
  ...
  const podeEditar = isSuperAdmin || isDono || perfil === "Administrador";
  ...
  const carregar = async (m: string) => {
    if (!wsId) { setRegistros([]); setCarregando(false); return; }
    setCarregando(true);
  ...
  const salvarEdit = async () => {
    ...
    try {
      if (editModal.modo === "editar" && editModal.registroId) {
        ...
        }).eq("id", editModal.registroId).eq("workspace_id", wsId);
  ...
  const excluirBatida = async (b: Registro) => {
    ...
    try {
      const { error } = await supabase.from("ponto_registros").delete().eq("id", b.id).eq("workspace_id", wsId);
      if (error) throw error;
      await carregar(mes);
    } catch (e: any) {
