...
export function HoleritesSection() {
  ...
  const carregar = async () => {
    ...
    setCarregando(true);
    const { data, error } = await supabase.from("holerites").select("*").eq("workspace_id", wsId).order("nome", { ascending: true });
    if (error) {
      console.error(error);
  ...
  useEffect(() => {
    if (!wsId) return;
    (async () => {
      const { data } = await supabase.from("folha_itens").select("competencia").eq("workspace_id", wsId);
  ...
  const gerarHolerites = async () => {
    ...
    const { data: jaTem } = await supabase.from("holerites").select("nome").eq("workspace_id", wsId).eq("competencia", compGerar);
