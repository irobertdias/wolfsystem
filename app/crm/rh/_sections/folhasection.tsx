...
export function FolhaSection() {
  ...
  const carregar = async (manterComp?: string) => {
    ...
    setCarregando(true);
    const { data, error } = await supabase.from("folha_itens").select("*").eq("workspace_id", wsId).order("nome", { ascending: true });
    if (error) {
      console.error(error);
  ...
  const marcarPagas = async () => {
    ...
    setProcessando(true);
    const { error } = await supabase.from("folha_itens").update({ status: "pago" }).eq("workspace_id", wsId).eq("competencia", comp);
