...
export function BancoHorasSection() {
  ...
  const carregar = async (m: string) => {
    ...
    const [resPonto, resManual] = await Promise.all([
      ...
      supabase.from("banco_horas").select("*").eq("workspace_id", wsId).order("data", { ascending: false }),
    ]);
  ...
  const excluir = async (l: Lanc) => {
    ...
    const { error } = await supabase.from("banco_horas").delete().eq("id", l.id).eq("workspace_id", wsId);
    if (error) {
      alert("Erro: " + error.message);
      return;
    }
