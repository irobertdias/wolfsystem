...
export function CandidatosSection() {
  ...
  const mudarEtapa = async (c: Candidato, etapa: string) => {
    const { error } = await supabase.from("candidatos").update({ etapa }).eq("id", c.id).eq("workspace_id", wsId);
    if (error) {
      alert("Erro: " + error.message);
      return;
    }
  ...
  const excluir = async (c: Candidato) => {
    ...
    const { error } = await supabase.from("candidatos").delete().eq("id", c.id).eq("workspace_id", wsId);
    if (error) {
      alert("Erro: " + error.message);
      return;
    }
