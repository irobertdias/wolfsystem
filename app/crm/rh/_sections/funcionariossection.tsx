...
// ═══════════════════════════════════════════════════════════════════════
// 🧑‍💼 RH · Funcionários  (WolfSystem — multi-tenant · tabela 'funcionarios')
// ───────────────────────────────────────────────────────────────────────
// PORTADO DO UNITA → WOLF. Isolamento por workspace_id em TODAS as queries:
//   • select  → .eq("workspace_id", wsId)
//   • insert  → carimba workspace_id: wsId
//   • update  → .eq("id", id).eq("workspace_id", wsId)
//   • delete  → .eq("id", id).eq("workspace_id", wsId)
// O select de login vinculado vem de 'usuarios_workspace' (não 'usuarios'),
// também filtrado pelo workspace. 'admissao' vazio vira null (coluna date).
// ═══════════════════════════════════════════════════════════════════════
...
type Funcionario = {
  ...
  salario: number;
  status: Status;
  user_email: string; // login do sistema vinculado (usuarios_workspace.email) — usado no ponto
};
...
export function FuncionariosSection() {
  ...
  // logins do sistema (usuarios_workspace) — pro select de vínculo do funcionário
  ...
  // 🔌 Carrega da tabela (isolado por workspace)
  ...
  // 🔌 carrega os logins do workspace (usuarios_workspace) pro select de vínculo
  useEffect(() => {
    if (!wsId) return;
    (async () => {
  ...
  // 🔌 departamentos e equipes do workspace (pros selects do cadastro de funcionário)
  useEffect(() => {
    if (!wsId) return;
    (async () => {
      const [dep, eq] = await Promise.all([
        supabase.from("departamentos").select("id, nome").eq("workspace_id", wsId).order("nome", { ascending: true }),
  ...
  const filtrados = useMemo(() => {
    ...
    if (filtroEquipe !== "todos") l = l.filter((f) => f.equipe_id === filtroEquipe);
  ...
  // 🔌 insert / update (carimba/filtra workspace)
  const salvar = async () => {
    if (!form.nome.trim()) {
                    ...
                    >
                      ...
                      <td style={{ padding: "12px 16px", color: "#4b5563", fontSize: 12 }}>
                        ...
                        {f.equipe_id && equipeNome(f.equipe_id) ? (
                          <span style={{ display: "block", color: "#6366f1", fontSize: 11, fontWeight: 600 }}>👥 {equipeNome(f.equipe_id)}</span>
                        ) : null}
                      </td>
                      <td
