...
export default function CRMLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      else setWorkspaceNome("Administrador");
      if (admin) {
      ...
      <div style={{ width: 220, background: "#111", borderRight: "1px solid #1f2937", display: "flex", flexDirection: "column", padding: 16, gap: 8, flexShrink: 0 }}>
        ...
        {menuItems.map(item => (
          <button key={item.path} onClick={() => router.push(item.path)}
            ...
            <span>{item.icon}</span>
            {item.label}
            {item.badge > 0 && <span style={{ background: "#16a34a", color: "white", borderRadius: 10, padding: "1px 6px", fontSize: 10, marginLeft: "auto" }}>{item.badge}</span>}
          </button>
        ))}
