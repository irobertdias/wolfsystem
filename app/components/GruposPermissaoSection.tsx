...
export const CATEGORIAS_PERMISSAO_WOLF: CategoriaPermissao[] = [
  ...
  },
  {
    ...
    permissoes: [
      ...
      {
        key: "contratos_criar",
        label: "Criar e enviar contratos para assinatura",
      },
      {
        key: "contratos_editar",
        label: "Editar contratos e criar novas versões",
      },
      ...
      {
        key: "contratos_configurar",
        label: "Configurar empresa e representantes",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    permissoes: [
      ...
      { key: "voip_ura_atender", label: "Atender chamadas e leads da URA" },
  ...
  },
  {
    nome: "💰 Cobrança",
    cor: "#dc2626",
    permissoes: [{ key: "cobranca", label: "Acessar a Cobrança (tudo)" }],
  },
  {
    nome: "🧑‍💼 RH",
    cor: "#4f46e5",
  ...
  },
  {
    nome: "🕐 Bater Ponto",
    cor: "#db2777",
    permissoes: [{ key: "bater_ponto", label: "Acessar o Bater Ponto" }],
  },
  {
  ...
  },
  {
    nome: "👤 Pessoal",
    cor: "#0ea5e9",
    permissoes: [{ key: "config_proprio", label: "Editar próprio perfil" }],
  },
];

// Exporta lista FLAT pra usar em outros lugares (defaults, validação, etc)
export const TODAS_PERMISSOES_WOLF = CATEGORIAS_PERMISSAO_WOLF.flatMap(
  (c) => c.permissoes,
);

// Mapa SLUG→LABEL (pra mostrar nome amigável em outros lugares)
export const LABELS_PERMISSOES_WOLF: Record<string, string> =
  TODAS_PERMISSOES_WOLF.reduce(
    (acc, p) => {
      acc[p.key] = p.label;
      return acc;
    },
...
export const PERMISSOES_VAZIO: Record<string, boolean> =
  TODAS_PERMISSOES_WOLF.reduce(
    (acc, p) => {
      acc[p.key] = false;
      return acc;
...
export default function GruposPermissaoSection({
  ...
  onChange,
  readOnly = false,
  categorias = CATEGORIAS_PERMISSAO_WOLF,
  estilo,
}: Props) {
  ...
  const categoriasFiltradas = useMemo(() => {
    ...
    return categorias
      .map((c) => ({
        ...c,
        permissoes: c.permissoes.filter(
          (p) =>
            p.label.toLowerCase().includes(b) ||
            p.key.toLowerCase().includes(b),
        ),
      }))
  ...
  const setarTudo = (novo: boolean) => {
    ...
    const next = { ...value };
    for (const c of categorias)
      for (const p of c.permissoes) next[p.key] = novo;
    onChange(next);
  };

  return (
    <div
      style={{ display: "flex", flexDirection: "column", gap: 14, ...estilo }}
    >
      ...
      >
        ...
        >
          ...
          <div>
            ...
            >
              {totalAtivas} <span style={{ color: "#9ca3af" }}>de</span>{" "}
              {totalGeral}{" "}
            ...
            <p style={{ margin: "2px 0 0", fontSize: 11, color: "#9ca3af" }}>
              {readOnly
                ...
                : "Marque o que esse grupo pode fazer"}
            </p>
          </div>
        </div>
        ...
        {!readOnly && (
          <div style={{ display: "flex", gap: 6 }}>
            <button
              onClick={() => setarTudo(true)}
              style={btnGlobal("#16a34a", "#f0fdf4", "#bbf7d0")}
            >
              ✓ Tudo
            </button>
            <button
              onClick={() => setarTudo(false)}
              style={btnGlobal("#dc2626", "#fef2f2", "#fecaca")}
            >
              ✕ Nada
            </button>
          </div>
        ...
        categoriasFiltradas.map((cat) => {
          ...
          return (
            ...
            >
              ...
              <div
                onClick={() =>
                  !busca && setCategoriaAberta(aberta ? null : cat.nome)
                }
                style={{
                  display: "flex",
                  alignItems: "center",
              ...
              >
                ...
                {!readOnly && (
                  ...
                  >
                    <button
                      onClick={() => setarCategoria(cat, true)}
                      title={`Marcar tudo de ${cat.nome}`}
                      style={btnPequeno("#16a34a")}
                    >
                      ✓
                    </button>
                    <button
                      onClick={() => setarCategoria(cat, false)}
                      title={`Desmarcar tudo de ${cat.nome}`}
                      style={btnPequeno("#dc2626")}
                    >
                      ✕
                    </button>
                  </div>
                  ...
                  {cat.permissoes.map((p) => {
                    ...
                    const labelFinal =
                      indent === 0
                        ...
                        : indent === 1
                          ? `↳ ${labelLimpo}`
                          : `✏️ editar ${labelLimpo.replace(/^editar\s*/, "")}`;
                    return (
                      <label
                        key={p.key}
                        style={{
